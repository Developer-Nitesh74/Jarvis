$(document).ready(function () {
    ...
    // Shared stop logic — switches screen immediately in JS, then tells Python to stop
    window.goToMainScreen = function () {
        // 1. Switch screens immediately (pure JS, no waiting for Python)
        $("#SiriWave").attr("hidden", true);
        $("#Oval").attr("hidden", false);
        $("#MicBtn").attr('hidden', false);
        $("#SendBtn").attr('hidden', true);

        // 2. Tell Python to kill speech (in background, don't wait)
        try { eel.stopJarvis()(); } catch(e) {}
    };
    ...
    // Stop button on full SiriWave screen (also bound via onclick in HTML)
    window.handleSiriStop = function () {
        window.goToMainScreen();
    };
